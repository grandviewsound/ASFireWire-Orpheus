...
enum class Register32 : uint32_t {
    ...
    // Sentinel value to widen the enum range for computed DMA context register offsets
    // (see `DMAContextHelpers`). This silences clang-analyzer's EnumCastOutOfRange warnings
    // while keeping strongly typed register access at call sites.
...
[[nodiscard]] constexpr Register32 Register32FromOffsetUnchecked(uint32_t offset) noexcept {
    ...
    return static_cast<Register32>(offset);
}
...
struct LinkControlBits {
    ...
    // Optional fields
    // static constexpr uint32_t kCycleSource = 1u << <bit>;      ///< rsc(u=*): external cycle
...
struct IntEventBits {
    ...
    static constexpr uint32_t kARRQ =
        1u << 2; // Asynchronous Receive Request DMA interrupt. This bit is conditionally set upon
...
struct DMAContextHelpers {
    ...
    static constexpr uint32_t IsoXmitContextControl(uint32_t n) {
        return 0x200u + 16u * n;
    } // For READS
    static constexpr uint32_t IsoXmitContextControlSet(uint32_t n) {
        return 0x200u + 16u * n;
    } // For WRITES (set bits)
    static constexpr uint32_t IsoXmitContextControlClear(uint32_t n) {
        return 0x204u + 16u * n;
    } // For WRITES (clear bits)
