...
// Include OHCIConstants.hpp for IEEE 1394 wire format constants (single source of truth)
#include "../Core/OHCIConstants.hpp"

/// OHCI Hardware Specification Structures
///
/// This header defines data structures and helper functions that directly correspond to the
/// IEEE 1394 Open Host Controller Interface (OHCI) Specification 1.1.
///
...
// OHCI §7.1.5.1 stores the descriptor address in bits [31:4] of CommandPtr/branchWord,
// leaving the lower 4 bits for the Z field. Guard this assumption at compile time so
// future refactors do not silently break the encoding helpers.
...
static_assert(kOHCIDmaAddressBits == 32,
              "OHCI DMA only supports 32-bit physical addresses (see §7.1.5.1)");
static_assert(kOHCIBranchAddressBits == 28,
              "BranchWord encoding hard-codes 28 address bits (bits [31:4]); "
              "update MakeBranchWordAT/DecodeBranchPhys32 if the spec changes.");
